...
if [ ! -z "$KASM_PROFILE_LDR" ]; then
    ...
    if [ -z "$kasm_profile_sync_found" ]; then
        ...
        http_proxy="" https_proxy="" /usr/bin/kasm-profile-sync --upload /home/${KASM_USER} --insecure --filter "${KASM_PROFILE_FILTER}" --remote ${KASM_API_HOST} --port ${KASM_API_PORT} -c ${KASM_PROFILE_CHUNK_SIZE} --token ${KASM_API_JWT}
